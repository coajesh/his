<template>
	<div>
		...
		<el-breadcrumb separator-class="el-icon-arrow-right">
		  ...
		  <el-breadcrumb-item>药房退药</el-breadcrumb-item>
		</el-breadcrumb>
		<!-- 卡片区域 -->
			...
			<el-dialog
				...
				<el-form label-width="80px">
					...
					<el-form-item>
						...
						<el-button @click="refundMedicine">退药</el-button>
					</el-form-item>
				</el-form>
			</el-dialog>
		</el-card>
	...
	export default{
		...
		methods: {
			...
			async getRegisterList(){
				this.register_list=await this.$axios.get("http://localhost:8080/drug/refundList").then(res => res.data)
			},
			// 根据id查找病人处方
			async check(id){
			...
			//退药操作
			async refundMedicine(){
				this.prescriptionDialogVisible=false
				await this.$axios.get("http://localhost:8080/drug/change?id="+this.prescription.id+"&state=-1")
